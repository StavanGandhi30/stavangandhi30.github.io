...
const DEFAULT_EXP_FIGURE_CLASS =
  "mt-6 overflow-hidden rounded-xl border border-stone-200";

const EXP_FIGCAPTION_CLASS =
  "border-t border-stone-200 bg-stone-50 px-3 py-2.5 text-[11px] leading-relaxed text-stone-500";

const EXP_VIDEO_CLASS = "max-h-[min(28rem,75vh)] w-full object-contain";
const EXP_VIDEO_CLASS_CAROUSEL =
  "max-h-[min(28rem,75vh)] w-full bg-black object-contain";
let siteUrlsPromise;
...
function expFigcaptionEsc(captionEscaped) {
  return captionEscaped
    ? `<figcaption class="${EXP_FIGCAPTION_CLASS}">${captionEscaped}</figcaption>`
    : "";
}
...
function renderExpVideoFigure(
  figureClass,
  ariaEscaped,
  srcEscaped,
  attrs,
  captionEscaped,
  videoClass = EXP_VIDEO_CLASS,
) {
...
function bindRecommendationCardStack(stack) {
  ...
  cards.forEach((card) => {
    ...
    const bringToFront = () => {
      cards.forEach((c) =>
        c.classList.remove("recommendation-stack-card--front"),
      );
...
function renderExperienceCarouselSlide(slide, expIndex, slideIndex, siteUrls) {
  ...
  if (slide.type === "video") {
    const src = safeHttpUrl(
      resolveSiteUrlFromRecord(siteUrls, slide, "srcKey", "src"),
    );
    if (!src) return "";
    const aria = escapeHtml(
      String(
        slide.ariaLabel ||
          `Experience clip ${expIndex + 1} · slide ${slideIndex + 1}`,
      ),
    );
    return renderExpVideoFigure(
      figCls,
      aria,
      escapeHtml(src),
      expPosterLoopAttrs(slide),
      capEsc,
      EXP_VIDEO_CLASS_CAROUSEL,
    );
  }
...
function renderExperienceCarousel(media, index, siteUrls) {
  ...
  const figureClass = escapeHtml(
    String(media.figureClass || DEFAULT_EXP_FIGURE_CLASS),
  );
  const aria = escapeHtml(
    String(media.ariaLabel || `Experience media ${index + 1}`),
  );
  const navBtns =
    n > 1
...
function renderExperienceMedia(media, index, siteUrls) {
  ...
  if (media.type === "video") {
    const src = safeHttpUrl(
      resolveSiteUrlFromRecord(siteUrls, media, "srcKey", "src"),
    );
    if (!src) return "";
    const aria = escapeHtml(
      String(media.ariaLabel || `Experience clip ${index + 1}`),
    );
    return renderExpVideoFigure(
      figureClass,
      aria,
      escapeHtml(src),
      expPosterLoopAttrs(media),
      captionEsc,
    );
  }
...
function renderExperienceLinks(links, siteUrls) {
  ...
  const linkClass =
    "underline decoration-stone-300 underline-offset-4 transition hover:decoration-stone-600";
  const anchors = items
    .map((item) => {
      const href = safeHttpUrl(
        resolveSiteUrlFromRecord(siteUrls, item, "hrefKey", "href"),
      );
  ...
  return `<p class="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-stone-600">${anchors.join(
...
export async function initExperience() {
  ...
  root.innerHTML =
    '<p class="text-sm text-stone-400" data-exp-loading>Loading…</p>';
  ...
  try {
    ...
    root.innerHTML =
      '<p class="text-sm text-stone-600">Experience couldn’t load. Serve <code class="text-xs">dist/</code> over HTTP (not <code class="text-xs">file://</code>) so <code class="text-xs">data/experience.json</code> can be fetched, or check that the file exists and is valid JSON.</p>';
    rafFade();
    return;
  }
  ...
  try {
    ...
    if (!items.length) {
      root.innerHTML =
        '<p class="text-sm text-stone-500">No experience entries yet.</p>';
      rafFade();
      return;
    }
    ...
    root.innerHTML = items
      .map((item, i) => {
        ...
        const bulletsHtml = bullets.length
          ? `<ul class="mt-6 space-y-3.5 border-l-2 border-stone-200 pl-5 text-sm leading-relaxed text-stone-600">${bullets
              .map((b) => {
                const title = escapeHtml(String(b?.title || "").trim());
                const text = escapeHtml(String(b?.text || "").trim());
                if (!text) return "";
                return `<li>${title ? `<span class="font-medium text-stone-900">${title}</span> —` : ""}${text}</li>`;
              })
              .filter(Boolean)
              .join("")}</ul>`
          : "";

        return `<li class="border-b border-stone-200 pb-14 last:border-0 last:pb-0">
 <h3 class="font-serif mt-6 text-xl text-stone-900">${org}</h3>
 <div class="flex flex-col gap-1 sm:flex-row sm:items-baseline sm:justify-between">
 <p class="mt-1 text-sm text-stone-500">${role}</p>
 <p class="mt-1 text-sm text-stone-500">${date}</p>
 </div>
 ${linksHtml}
 ${mediaHtml}
 ${description ? `<p class="mt-4 text-sm leading-relaxed text-stone-600">${description}</p>` : ""}
 ${bulletsHtml}
 </li>`;
      })
      .join("");
    ...
    root.innerHTML =
      '<p class="text-sm text-stone-600">Experience loaded but couldn’t be displayed. Check <code class="text-xs">data/experience.json</code> for valid JSON.</p>';
    rafFade();
  }
}

const EDU_PROG_LINK_CLASS =
  "ml-1 whitespace-nowrap text-stone-800 underline decoration-stone-300 underline-offset-4 hover:decoration-stone-600";
...
export async function initEducation() {
  ...
  root.innerHTML =
    '<p class="text-sm text-stone-400" data-edu-loading>Loading…</p>';
  ...
  try {
    ...
    root.innerHTML =
      '<p class="text-sm text-stone-600">Education couldn’t load. Serve <code class="text-xs">dist/</code> over HTTP (not <code class="text-xs">file://</code>) so <code class="text-xs">data/education.json</code> can be fetched, or check that the file exists and is valid JSON.</p>';
    rafFade();
    return;
  }
  ...
  try {
    ...
    if (!items.length) {
      root.innerHTML =
        '<p class="text-sm text-stone-500">No education entries yet.</p>';
      rafFade();
      return;
    }
    ...
    root.innerHTML = `<div class="mt-8 space-y-12">${items
      .map((item) => {
        ...
        const highlightsHtml = highlights.length
          ? `<ul class="mt-6 space-y-1.5 text-sm leading-snug text-stone-600">
 ${highlights
   .map((h) => {
     const t = escapeHtml(String(h || "").trim());
     return t ? `<li><span class="text-stone-900">${t}</span></li>` : "";
   })
   .filter(Boolean)
   .join("")}
 </ul>`
 ...
 decoding="async"
 />
 <p class="font-serif text-xl text-stone-900">${school}</p>
 </div>
 <p class="mt-1 text-stone-600">${degree}</p>
 <p class="mt-2 text-sm text-stone-500">${dates}</p>
 <p class="mt-5 text-sm leading-relaxed text-stone-600">${desc}${progA ? ` ${progA}` : ""}</p>
 ${highlightsHtml}
 </div>`;
      })
      .join("")}</div>`;
    ...
    root.innerHTML =
      '<p class="text-sm text-stone-600">Education loaded but couldn’t be displayed. Check <code class="text-xs">data/education.json</code> for valid JSON.</p>';
    rafFade();
  }
}
...
export async function initCredentials() {
  ...
  root.innerHTML =
    '<p class="text-sm text-stone-400" data-certs-loading>Loading…</p>';
  ...
  try {
    ...
    root.innerHTML =
      '<p class="text-sm text-stone-600">Credentials couldn’t load. Serve <code class="text-xs">dist/</code> over HTTP (not <code class="text-xs">file://</code>) so <code class="text-xs">data/credentials.json</code> can be fetched, or check that the file exists and is valid JSON.</p>';
    rafFade();
    return;
  }
  ...
  try {
    ...
    if (!items.length) {
      root.innerHTML =
        '<p class="text-sm text-stone-500">No credentials listed yet.</p>';
      rafFade();
      return;
    }

    const certLinkClass =
      "text-[13px] text-stone-700 underline decoration-stone-300 underline-offset-4";
    root.innerHTML = items
      .map((item) => {
        const title = escapeHtml(item.title || "");
        const meta = escapeHtml(item.meta || "");
        const desc = escapeHtml(String(item.description || "").trim());
        const certHref = safeHttpUrl(
          resolveSiteUrlFromRecord(siteUrls, item, "urlKey", "url"),
        );
        const courseHref = safeHttpUrl(
          resolveSiteUrlFromRecord(siteUrls, item, "courseUrlKey", "courseUrl"),
        );
        ...
        if (certA || courseA) {
          const sep =
            certA && courseA
              ? '<span class="text-stone-300 select-none" aria-hidden="true"> · </span>'
              : "";
          linksRow = `<div class="mt-4 flex flex-wrap items-center gap-x-1 gap-y-1">${courseA}${sep}${certA}</div>`;
        }
        return `
 <article class="certificate-card">
 <h3 class="font-serif mt-6 text-xl text-stone-900">${title}</h3>
 <p class="mt-1 text-sm text-stone-500">${meta}</p>
 <div class="certificate-card__desc mt-4" data-cert-desc-block>
 <p class="text-sm leading-relaxed text-stone-600 certificate-card__para"><span class="certificate-card__text">
 ${desc}
</span><button
 type="button"
 class="certificate-card__more-btn ml-1 inline align-baseline border-0 bg-transparent p-0 text-left text-[13px] font-medium leading-relaxed text-stone-600 underline decoration-stone-300 decoration-1 underline-offset-[0.2em] transition hover:text-stone-900 hover:decoration-stone-500"
 hidden
 aria-expanded="false"
 ...
 ${linksRow}
    ...
    root.innerHTML =
      '<p class="text-sm text-stone-600">Credentials loaded but couldn’t be displayed. Check <code class="text-xs">data/credentials.json</code> for valid JSON.</p>';
    rafFade();
  }
}
...
export async function initRecommendations() {
  ...
  root.innerHTML =
    '<p class="text-sm text-stone-400" data-rec-loading>Loading…</p>';
  ...
  try {
    ...
    root.innerHTML =
      '<p class="text-sm text-stone-600">Endorsements couldn’t load. Serve <code class="text-xs">dist/</code> over HTTP so <code class="text-xs">data/recommendations.json</code> can be fetched.</p>';
    return;
  }
  ...
  try {
    ...
    if (!items.length) {
      root.innerHTML =
        '<p class="text-sm text-stone-500">No endorsements listed yet.</p>';
      return;
    }
 ...
 <p class="recommendation-stack-card__quote">${quoteHtml}</p>
    ...
    if (discEl) discEl.setAttribute("hidden", "");
    root.innerHTML =
      '<p class="text-sm text-stone-600">Endorsements loaded but couldn’t be displayed. Check <code class="text-xs">data/recommendations.json</code>.</p>';
  }
}
...
export async function initBlog() {
  ...
  root.innerHTML =
    '<p class="text-sm text-stone-400" data-blog-loading>Loading…</p>';
  ...
  try {
    ...
    root.innerHTML =
      '<p class="text-sm text-stone-600">Posts couldn’t load. Serve <code class="text-xs">dist/</code> over HTTP (not <code class="text-xs">file://</code>) so <code class="text-xs">data/blogs.json</code> can be fetched, or check that the file exists and is valid JSON.</p>';
    rafFade();
    return;
  }
  ...
  try {
    ...
    root.innerHTML = posts
      .map((p) => {
        ...
        return `
 <article id="${hash}" class="scroll-mt-28 border border-stone-200 bg-white">
 <details class="blog-post group">
 <summary class="cursor-pointer p-6 transition hover:bg-stone-50">
 <div class="flex flex-wrap items-start justify-between gap-3">
 <div class="min-w-0 flex-1">
 <time class="text-[11px] font-medium uppercase tracking-[0.2em] text-stone-400">${formatPostDate(p.date)}</time>
 <h3 class="font-serif mt-3 text-lg font-normal text-stone-900">${escapeHtml(p.title)}</h3>
 <p class="mt-2 text-sm leading-relaxed text-stone-600">${escapeHtml(p.excerpt || "")}</p>
 </div>
 <span class="blog-expand-label shrink-0 text-[11px] uppercase tracking-wider text-stone-500">Expand</span>
 </div>
 <div class="mt-4 flex flex-wrap gap-2">
 ${(p.tags || [])
   .map(
     (t) =>
       `<span class="text-[10px] uppercase tracking-wider text-stone-500">${escapeHtml(t)}</span>`,
   )
   .join(' <span class="text-stone-300" aria-hidden="true">·</span>')}
 </div>
 </summary>
 <div class="border-t border-stone-200 px-6 pb-6 pt-5">
 <div class="space-y-4 text-[15px] leading-[1.7] text-stone-700 max-w-none [&_p]:mb-4 [&_p:last-child]:mb-0 [&_strong]:font-semibold [&_em]:italic">${p.content}</div>
 <p class="mt-6 text-[11px] uppercase tracking-wider text-stone-400">Permalink <a class="text-stone-800 underline decoration-stone-300 underline-offset-4 hover:decoration-stone-600" href="#${hash}">#${hash}</a></p>
 </div>
 </details>
 </article>`;
      })
    ...
    if (intro) intro.setAttribute("hidden", "");
    root.innerHTML =
      '<p class="text-sm text-stone-600">Posts loaded but couldn’t be displayed. Check <code class="text-xs">data/blogs.json</code> for valid JSON.</p>';
    rafFade();
  }
}
...
export async function initLinkedInEmbeds() {
  ...
  track.innerHTML = '<p class="text-sm text-stone-400">Loading…</p>';
  let data;
  try {
    data = await loadSiteJson("data/linkedin-posts.json");
  } catch {
    track.innerHTML =
      '<p class="text-sm text-stone-600">LinkedIn posts couldn’t load. Serve <code class="text-xs">dist/</code> over HTTP so <code class="text-xs">data/linkedin-posts.json</code> can be fetched.</p>';
    rafFade();
    return;
  }
  ...
  if (!posts.length) {
    track.innerHTML =
      '<p class="text-sm text-stone-500">No LinkedIn posts yet.</p>';
    rafFade();
    return;
  }
  ...
  posts.forEach((post, i) => {
    const src = safeHttpUrl(
      resolveSiteUrlFromRecord(siteUrls, post, "srcKey", "src"),
    );
    if (!src) return;
    ...
    a.href =
      safeHttpUrl(
        resolveSiteUrlFromRecord(siteUrls, post, "hrefKey", "href"),
      ) || linkedInPostUrl(src, siteUrls);
    a.target = "_blank";
...
export async function initPhotos() {
  ...
  root.innerHTML = '<p class="text-sm text-stone-400">Loading…</p>';
  ...
  try {
    ...
    root.innerHTML =
      '<p class="text-sm text-stone-600">Photos couldn’t load. Serve <code class="text-xs">dist/</code> over HTTP so <code class="text-xs">data/photos.json</code> can be fetched.</p>';
    rafFade();
    return;
  }
  ...
  try {
    ...
    if (!items.length) {
      root.innerHTML =
        '<p class="text-sm text-stone-500">No photos yet. Add entries to <code class="text-xs">data/photos.json</code>.</p>';
      rafFade();
      return;
    }
    ...
    root.innerHTML = `<div class="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4 md:gap-5">${items
      .map((item) => {
        ...
        const capHtml = cap
          ? `<figcaption class="border-t border-stone-200 bg-stone-50 px-2 py-2 text-[11px] leading-snug text-stone-500">${escapeHtml(cap)}</figcaption>`
          : "";
        const srcEsc = escapeHtml(src);
        return `<figure class="m-0 overflow-hidden rounded-xl border border-stone-200 bg-stone-100">
 ...
 ${capHtml}
    ...
    root.innerHTML =
      '<p class="text-sm text-stone-600">Photos loaded but couldn’t be displayed. Check <code class="text-xs">data/photos.json</code>.</p>';
    rafFade();
  }
}
